...
{
    public class ExportServiceR : ExportServiceBase
    {
        private ExportSetting m_Setting;

        public ExportServiceR(ExportSetting settings, TextWriter wr)
        {
            m_Setting = settings;
            m_TextWriter = wr;
        }
        ...
        public override void ExportGrid(int nRows, int nCols, GridAccessor accessor, GridHeaderAccessor rhaccessor, GridHeaderAccessor chaccessor, string[] chtags = null)
        {
            ...
            for (int c = 0; c < nCols; c++)
            {
                if (tags_t[c] == '#')
                {
                    corpus_tokens[tags_n[c]] = chaccessor(c) as string;
                }
                else if (tags_t[c] == '?')
                {
                    word_tokens[tags_n[c]] = string.Format("w{0}", tags_n[c]);
                }
            }
            m_TextWriter.Write("\t");
